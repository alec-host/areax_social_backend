...
class ReportPostController {
   ...
   async reportedSocialPost(req,res){
	 ...
	 const postResponse = await getPostById(post_id);  
         if(!postResponse[0]){
            res.status(404).json({
                success: false,
                error: true,
                message: postResponse[1]
            });
            return;
         }	      
	 ...
	 const payload = { 
	    ...
	    feedback,
	    post_id,
	    media_url: postResponse[1].media_url,
	    description: postResponse[1].description,
            creator_email: postResponse[1].email,
	    creator_reference_number: postResponse[1].reference_number	 
	 };
   ...
   async reportedGroupPost(req,res){
      let redisClient = null;
      const errors = validationResult(req);
      const { email, reference_number, group_reference_number, feedback, post_id } = req.body;
      try{
         if(!errors.isEmpty()){
            res.status(422).json({ success: false, error: true, message: errors.array() });
            return;
