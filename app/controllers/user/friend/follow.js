...
async function getConnectionFollowList(userReferenceNumber, page = 1, limit = 10) {
  const cacheKey = `user:follow:connections:${userReferenceNumber}:page:${page}:limit:${limit}`;
  const client = await connectToRedis();
  const offset = (page - 1) * limit;
  try {
      const cached = await getCache(client, cacheKey);
      if(cached) return [true, cached];	  
      const [followUserList] = await Promise.all([
...
module.exports = { getConnectionFollowList };
